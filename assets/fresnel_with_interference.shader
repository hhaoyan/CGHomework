...
void main() {
    ...
    cosine = abs(dot(I, worldNormal));
    thickness = 0.5;//(position.y + 0.5);//(position.y + 0.5) / 2.0 * 0.5 ;
}
...
uniform sampler2D tInterference;
...
void main() {
    ...
    reflectedColor *= texture(tInterference, vec2(cosine, thickness));
